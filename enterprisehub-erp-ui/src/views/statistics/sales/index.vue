<template>
  <n-card title="销售统计" :bordered="false">
    <n-space vertical :size="16">
      ...
      <n-grid cols="4" x-gap="16">
        <n-gi>
          <n-statistic label="销售总额" :value="statistics.totalAmount">
            <template #prefix>¥</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="订单数量" :value="statistics.orderCount">
            <template #suffix>单</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="客户数量" :value="statistics.customerCount">
            <template #suffix>个</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="平均订单额" :value="statistics.avgAmount">
            <template #prefix>¥</template>
          </n-statistic>
        </n-gi>
      </n-grid>
...
const columns: DataTableColumns = [
  ...
  { title: '客户数', key: 'customerCount', width: 120 },
  { title: '平均订单额', key: 'avgAmount', width: 150 }
]
...
const handleQuery = async () => {
  loading.value = true
  try {
    const res: any = await request.get('/statistics/v1/sales', { params: queryParams })
