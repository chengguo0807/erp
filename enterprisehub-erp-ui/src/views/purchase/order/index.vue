    ...
    <n-card title="采购订单管理">
      <n-space vertical :size="16">
        <n-space justify="space-between">
          <n-space>
            <n-input v-model:value="queryParams.orderNo" placeholder="订单编号" clearable style="width: 200px" />
            <n-select v-model:value="queryParams.status" placeholder="订单状态" clearable style="width: 150px" :options="statusOptions" />
    ...
    <n-modal v-model:show="showModal" :title="modalTitle" preset="card" style="width: 900px">
      <n-form ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="100px">
        <n-grid :cols="2" :x-gap="24">
          <n-form-item-gi label="供应商" path="supplierId">
            <n-select v-model:value="formData.supplierId" placeholder="请选择供应商" :options="supplierOptions" />
          </n-form-item-gi>
          <n-form-item-gi label="订单日期" path="orderDate">
            <n-date-picker v-model:value="formData.orderDate" type="date" style="width: 100%" />
          </n-form-item-gi>
          <n-form-item-gi label="预计到货日期" path="expectedDate">
            <n-date-picker v-model:value="formData.expectedDate" type="date" style="width: 100%" />
          </n-form-item-gi>
...
import { getPurchaseOrderList, addPurchaseOrder, updatePurchaseOrder, deletePurchaseOrder, auditPurchaseOrder } from '@/api/purchase'
...
const rules = {
  ...
  orderDate: { required: true, type: 'number', message: '请选择订单日期', trigger: 'change' }
}
...
const loadSuppliers = async () => {
  try {
    const res = await supplierApi.getList({ pageNum: 1, pageSize: 100 })
    supplierOptions.value = res.data.records.map((item: any) => ({
      label: item.name,
      value: item.id
    }))
  } catch (error) {
